...
const filePath = 'd:/Al-Akhlaq Lil Banin/src/data/chapters.json';
const ch9Path = 'd:/Al-Akhlaq Lil Banin/tmp/chapter_9.json';
const ch10Path = 'd:/Al-Akhlaq Lil Banin/tmp/chapter_10.json';
const ch11Path = 'd:/Al-Akhlaq Lil Banin/tmp/chapter_11.json';
...
const ch10 = JSON.parse(fs.readFileSync(ch10Path, 'utf8'));
const ch11 = JSON.parse(fs.readFileSync(ch11Path, 'utf8'));

// Find start of ID 9
const id9Match = content.match(/\{\s*"id":\s*9/);
if (!id9Match) {
  console.error("Could not find ID 9");
  process.exit(1);
}

const posId9 = id9Match.index;
const posStart9 = content.lastIndexOf('{', posId9);

let prefix = content.substring(0, posStart9).trim();
if (prefix.endsWith(',')) {
  prefix = prefix.substring(0, prefix.length - 1).trim();
}

const output = prefix + '\n  ' + 
  JSON.stringify(ch8_correct, null, 2) + ',\n  ' + 
  JSON.stringify(ch9, null, 2) + ',\n  ' + 
  JSON.stringify(ch10, null, 2) + ',\n  ' + 
  JSON.stringify(ch11, null, 2) + '\n]';

fs.writeFileSync(filePath, output, 'utf8');
console.log("Successfully fixed chapters.json");
